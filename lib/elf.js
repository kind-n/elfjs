...
function requireFormat (value) {
    return amdOptions.module === "commonjs" ? "define(" + requireDepend(value) + ",function(require,exports,module){" + value + "})" : value;
}
    ...
    if (connate === present) {
        if (emitter.contain(event.type)) {
            emitter.trigger(event.type,
                cliApply(example, Object.create(event, {
                    currentTarget : newProp(example, true)
                }))
            );
        }
    }
...
var trustorProto = {
    js   : function (value, modname) {
        return new Function("define", requireFormat(value))(requireDefine(modname)), amdModules[modname];
    },
