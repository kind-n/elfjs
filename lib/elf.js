...
function cliReset (target, value) {
    target.splice.apply(target, [0, target.length].concat(value));
}
    ...
    if (actLeisure) {
        ...
        setTimeout(function () {
            actMonitor.forEach(function (i) {
                i.drawn && i.duplex && i.forceUpdate();
            });
            actLeisure = true;
        });
    }
...
function domCreateElement (trustor, namespace) {
    return namespace ? doc.createElementNS(namespace, trustor) : doc.createElement(cliLower(trustor));
}
    ...
    if (newly) {
        members.push([
            ...
            example = function (event) {
                defProp(event, "refresh", true);
                defProp(event, "preventRefresh", exhibitEventRefersh);
                if (isObject(listener)) {
                    listener.handleEvent(event);
                } else {
                    listener.call(this, event);
                }
                event.refresh && actBroadcast();
            }
        ]);
    }
    return example;
...
function expName (target) {
    return function (name) {
        return target[name] || componentDep[name] || name;
    };
}
 ...
 * @returns 
 */
function promiseGain (value, resolve, reject) {
    try {
 ...
 * @param {any} natures 
 * @param {Array} behests 
 * @param {Array<Functioin>} collect
 */
function exhibitAppendOrder (command, example, natures, behests, collect) {
    for (var i = 0; i < command.length; i++) {
        var dictate = singlesCreator(command[i]);
        if (dictate[cycleInitial]) {
            collect.push(dictate[cycleInitial].bind(dictate, example, natures));
        }
 ...
 * @param {Array<Function>} collect
 */
function exhibitChangeOrder (command, example, natures, behests, collect) {
    var appends = [];
...
var promiseProto = {
    ...
    then         : function (onresolved, onrejected) {
        return promiseCreator(function (resolve, reject) {
            onresolved = onresolved || promiseNoop;
            onrejected = onrejected || promiseNoop;
            if (this[symbolProcess] === statusPending) {
                var clearResolved = cliPush(this[symbolSuccess], function (value) {
                    return promiseGain(value, ownResolved, ownAbnormal);
                });
                var clearRejected = cliPush(this[symbolFailure], function (error) {
                    return promiseGain(error, ownRejected, ownAbnormal);
                });
                return function () {
                    clearResolved();
                    clearRejected();
            ...
            if (this[symbolProcess] === statusResolved) {
                return promiseGain(this[symbolContent], ownResolved, ownAbnormal);
            }
            if (this[symbolProcess] === statusRejected) {
                return promiseGain(this[symbolContent], ownRejected, ownAbnormal);
            }
            function ownResolved (value) {
...
var complexProto = {
    initial : function (owner, namespace, collect) {
        ...
        exhibitAppendOrder(command, example, natures, behests, collect);
        exhibitAppendRefer(referee, example, owner);
    ...
    renewal : function (owner, namespace, collect, element) {
        ...
        exhibitChangeOrder(element.cmd, example, natures, behests, collect);
 ...
 * @returns {Elf.Disposable}
 */
Elf.setTimeout  = function (fn, delay) {
    return { dispose : clearTimeout.bind(win, setTimeout(cliWrap(fn, asArray.call(arguments, 2)), delay)) };
};
 ...
 * @returns {Elf.Disposable}
 */
Elf.setInterval = function (fn, delay) {
    return { dispose : clearInterval.bind(win, setInterval(cliWrap(fn, asArray.call(arguments, 2)), delay)) };
};
...
Elf.Transform = function (name, proto) {
    return defProp(isObject(proto) ? cliClass(proto) : proto, symbolTransform, name);
};
...
Elf.Directive = function (name, proto) {
    return defProp(isObject(proto) ? cliClass(proto) : proto, symbolDirective, name);
};
...
Elf.Component = function (name, proto) {
    return defProp(isObject(proto) ? cliClass(proto) : proto, symbolComponent, name);
};
...
Elf.render = function (element, container, duplex) {
    ...
    sapInsertion(realtor, container);
    sapExplosion(collect);
    example.drawn = true;
    return example;
};
