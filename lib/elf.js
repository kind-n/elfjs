...
function IS_VALID  (target) {
    return target !== void 0 && target !== null;
}
function IS_BASAL  (target) {
    return typeof target === "string" || typeof target === "number";
}
function IS_OBJECT (target) {
    return IS_VALID(target) && typeof target === "object";
}
function IS_STRING (target) {
    return IS_VALID(target) && typeof target === "string";
}
...
function FUN_PUSH  (target, value) {
    FUN_PLUS(target, value);
    return function () {
        FUN_LESS(target, value);
    };
}
function FUN_PLUS  (target, value) {
    if (target.indexOf(value) < 0) {
        return target.push(value) === 1;
    }
}
function FUN_LESS  (target, value) {
...
function XHR_ASYNC (request, referee) {
    ...
    DOM_ATTACH_EVENT(xhrRequest, TXT_LOAD, OWN_RESOLVED);
    DOM_ATTACH_EVENT(xhrRequest, TXT_FAIL, OWN_REJECTED);
    if (!HAS_PROP(xhrHeaders, xhrContent)) {
        XHR_AFFIX(xhrRequest, xhrContent, "application/x-www-form-urlencoded; charset=UTF-8");
    }
    if (!HAS_PROP(xhrHeaders, xhrPattern)) {
        XHR_AFFIX(xhrRequest, xhrPattern, "XMLHttpRequest");
    }
    for (var name in xhrHeaders) {
        XHR_AFFIX(xhrRequest, name, xhrHeaders[name]);
    }
    xhrRequest.send(request.body);
    return function () {
        DOM_DETACH_EVENT(xhrRequest, TXT_LOAD, OWN_RESOLVED);
        DOM_DETACH_EVENT(xhrRequest, TXT_FAIL, OWN_REJECTED);
        xhrRequest.abort();
    };
    function OWN_RESOLVED () {
        ...
        if (200 <= status && status < 300) {
            ...
            referee.loss(xhrRequest);
        }
        ACT_BROADCAST();
    }
    function OWN_REJECTED () {
        referee.loss(xhrRequest);
        ACT_BROADCAST();
    }
}
function XHR_JSONP (request, referee) {
    ...
    var xhrHostNode = DOM_CREATE_ELEMENT("script");
    WIN[xhrCallBack]= OWN_RESOLVED;
    DOM_ATTACH_EVENT(xhrHostNode, TXT_FAIL, OWN_REJECTED);
    DOM_ATTACH_PROPS(xhrHostNode, TXT_TYPE, "text/javascript");
    DOM_ATTACH_PROPS(xhrHostNode, TXT_HREF, request.url);
    DOM_APPEND_CHILD(HEAD, xhrHostNode);
    return function () {
        if (WIN[xhrCallBack]) {
        ...
        DOM_REMOVE_CHILD(xhrHostNode);
    };
    function OWN_RESOLVED (value) {
        delete WIN[xhrCallBack];
        referee.next({
            status  : 200,
            headers : {},
            text    : function () {
                return IS_OBJECT(value) ? TO_STRING(value) : value;
            },
            json    : function () {
                return value;
            }
...
function ACT_BROADCAST () {
    if (CONST_ACT_LEISURE) {
        ...
        setTimeout(function () {
            CONST_ACT_MONITOR.forEach(function (ns) {
                ns.duplex && ns.forceUpdate();
            });
...
function TPL_XHTML_PARSER (temp) {
    ...
    while (number < length) {
        if (OWN_COMMENT_START(number)) {
            OWN_COMMENT_PARSE(number, number = OWN_COMMENT_CLOSE(number + 4) + 3);
            continue;
        }
        if (OWN_CLOSURE_START(number)) {
            OWN_CLOSURE_PARSE(number, number = OWN_ELEMENT_CLOSE(number + 2) + 1);
            continue;
        }
        if (OWN_ELEMENT_START(number)) {
            OWN_ELEMENT_PARSE(number, number = OWN_ELEMENT_CLOSE(number + 1) + 1);
            continue;
        }
        OWN_CONTENT_PARSE(number, number = OWN_CONTENT_CLOSE(number));
    }
    if (roots.length > 1) {
        console.error("[Elf error]: Cannot use multiple root node.");
    }
    return roots[0] || "";
    function OWN_COMMENT_START (current) {
        return IS_STARTS(temp, TAG_COMMENT_OPEN, current);
    }
    function OWN_CLOSURE_START (current) {
        return IS_STARTS(temp, TAG_CLOSURE_OPEN, current) && REG_QNAME.test(temp.charAt(current + 2));
    }
    function OWN_ELEMENT_START (current) {
        return IS_STARTS(temp, TAG_ELEMENT_OPEN, current) && REG_QNAME.test(temp.charAt(current + 1));
    }
    function OWN_COMMENT_CLOSE (current) {
        current = temp.indexOf(TAG_COMMENT_OVER, current);
        return current >= 0 ? current : length;
    }
    function OWN_CONTENT_CLOSE (current) {
        current = temp.indexOf(TAG_ELEMENT_OPEN, current);
        return current >= 0 ? (
            OWN_COMMENT_START (current) ||
            OWN_CLOSURE_START (current) ||
            OWN_ELEMENT_START (current) ? current : OWN_CONTENT_CLOSE(current + 1)
        ) : length;
    }
    function OWN_ELEMENT_CLOSE (current) {
        return FUN_INDEX(temp, function () { return true; })(TAG_ELEMENT_OVER, current);
    }
    function OWN_COMMENT_PARSE (start, end) {
        console.warn("Comment '" + temp.slice(start, end) + "' will be dispensed.");
    }
    function OWN_CLOSURE_PARSE (start, end) {
    ...
    function OWN_ELEMENT_PARSE (start, end) {
    ...
    function OWN_CONTENT_PARSE (start, end) {
        OWN_STORE_VALUE(temp.slice(start, end).trim());
    }
    function OWN_STORE_VALUE (value, depth) {
        if (value) {
            queue[level] = value;
            if (level > 0) {
...
function GAN_NODE_ELEMENT (node) {
    if (IS_STRING(node)) {
        return GAN_NODE_CONTENT(node);
    }
    if (node.each) {
        if (node.level > 0) {
            return GAN_NODE_ITERATE(node);
        }
        console.error("[Elf error]: Cannot use elf-each on root node.");
    }
    if (node.when) {
        return GAN_NODE_TERNARY(node);
    }
    return GAN_NODE_DEFAULT(node);
}
function GAN_NODE_DEFAULT (node) {
    return "Elf.v(Elf.c(" + TO_STRING(node.qname) + ")," + GAN_NODE_NATURES(node) + (node.children.length ? "," + GAN_NODE_INSIDER(node) : "") + ")";
}
function GAN_NODE_ITERATE (node) {
    return "Elf.m(" + node.each.body + ",function(" + node.each.keys + "){with(this){return " + (node.when ? GAN_NODE_TERNARY(node) : GAN_NODE_DEFAULT(node)) + "}},this)";
}
function GAN_NODE_TERNARY (node) {
    return "((" + node.when + ")?" + GAN_NODE_DEFAULT(node) + ":null)";
}
function GAN_NODE_INSIDER (node) {
    return "[" + node.children.map(GAN_NODE_ELEMENT).join(",") + "]";
}
function GAN_NODE_CONTENT (node) {
    var number = 0;
    var incept = 0;
    var finish = -2;
    var result = [];
    ...
    function GAN_EXPRESSION (start, end) {
        var expr = GAN_NODE_FILTERS(UN_ESCAPE(node.slice(start, end)));
        if (expr) {
            result.push("(" + expr + ")");
        }
    }
}
function GAN_NODE_FILTERS (node) {
    var result;
    var number = -1;
    var divide = "|";
...
function GAN_NODE_NATURES (node) {
    return "{" +
        EXHIBIT_ENGINE_LOOPS(node.props, function (ns, name) {
            if (name === "cmd") {
                return TO_STRING(name) + ":Elf.d(" + GAN_NODE_CONTENT(ns) + ")";
            }
            if (IS_STARTS(name, EVENT_EXHIBIT_PREFIX)) {
                return TO_STRING(name) + ":function($event){with(this){" + ns + "}}.bind(this)";
            }
            return TO_STRING(name) + ":" + (ns ? GAN_NODE_CONTENT(ns) : TO_STRING(ns));
        }).join(",") +
    "}";
}

function SAP_INSERTION (realtor, container) {
    if (realtor.append) {
...
function SAP_EXPLOSION (collect) {
...
function TRUSTOR_TS   (value, modname) {
    return TRUSTOR_JS(ts.transpile(value, {
...
function TRUSTOR_CSS  (value, modname) {
    ...
    DOM_ATTACH_PROPS(node, TXT_TYPE, "text/css");
...
function EXHIBIT_JUDGE_REDRAWN (target, origin) {
    ...
    return IS_VALID(target) === IS_VALID(origin)
        && IS_BASAL(target) === IS_BASAL(origin);
}
...
function EXHIBIT_CHANGE_PROPS (ancient, natures, example, owner, namespace, current, connate) {
    ...
    EXHIBIT_DETACH_PROPS(remove, example, owner, namespace, current, connate);
    EXHIBIT_ATTACH_PROPS(append, example, owner, namespace, current, connate);
...
function EXHIBIT_SPREAD_EVENT (event, present) {
    ...
    if (connate === present) {
        if (emitter.contain(event.type)) {
            event = FUN_COPY({}, event, { currentTarget: example });
...
function RADICAL_CREATOR (element) {
    return {
        trigger : function (event, present) {
            if (present) {
                DOM_BUBBLE_EVENT(element, event.originalEvent);
            }
        }
    };
}
...
function EMITTER_CREATOR () {
    ...
    return {
        contain : function (type) {
            return HAS_EMITTER(type) && example[type].length > 0;
        },
        enqueue : function (type, value) {
            return DEF_EMITTER(type) && FUN_PLUS(example[type], value);
        },
        unqueue : function (type, value) {
            return HAS_EMITTER(type) && FUN_LESS(example[type], value);
        },
...
var CODE = DOM_CREATE_ELEMENT("div");
...
PROMISE_CREATOR.next = PROMISE_EXTERNAL_NEXT;
PROMISE_CREATOR.loss = PROMISE_EXTERNAL_LOSS;
PROMISE_CREATOR.when = PROMISE_EXTERNAL_WHEN;
PROMISE_CREATOR.race = PROMISE_EXTERNAL_RACE;
PROMISE_CREATOR.ajax = PROMISE_EXTERNAL_AJAX;
...
Elf.setTimeout    = function (fn, duration) {
    return { dispose : clearTimeout.bind(WIN, setTimeout(FUN_WRAP(fn), duration)) };
};
Elf.setInterval   = function (fn, duration) {
    return { dispose : clearInterval.bind(WIN, setInterval(FUN_WRAP(fn), duration)) };
};
Elf.setImmediate  = function (fn) {
    return { dispose : cancelAnimationFrame.bind(WIN, requestAnimationFrame(FUN_WRAP(fn))) };
};
...
Elf.redactElement = function (temp) {
    if (IS_STRING(temp)) {
        ...
        var trustor = new Function("Elf", "with(this){return " + GAN_NODE_ELEMENT(TPL_XHTML_PARSER(temp.trim())) + "}");
        return function () { return trustor.call(IS_VALID(this) && this !== WIN ? this : {}, provide); };
    }
    return temp;
};
Elf.forceUpdate   = function (duplex) {
    CONST_ACT_MONITOR.forEach(function (ns) {
        (duplex || ns.duplex) && ns.forceUpdate();
    });
};
...
Elf.render        = function (element, container, duplex) {
    var example = {
        ...
        forceUpdate : function (props) {
            SAP_INSERTION(manager.renewal(example, null, collect, element = Elf.createElement(element.type, props || element.props, element.children)), container);
            SAP_EXPLOSION(collect);
        }
    };
    var collect = [];
    var monitor = ACT_SUBSCRIBE(example);
    var creator = RADICAL_CREATOR(container);
    var manager = MANAGER_CREATOR(element, creator);
    SAP_INSERTION(manager.initial(example, null, collect), container);
    SAP_EXPLOSION(collect);
    return example;
};
Elf.depend        = function () {
